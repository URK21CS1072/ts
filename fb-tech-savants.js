...
db.collection("garbage_objects")
    ...
    .then((querySnapshot) => {
        if (!querySnapshot.empty) {
            ...
            console.log("No documents found in 'garbage_objects' collection.");
        }
    })
    .catch((error) => {
...
function createTable(data, docId) {
    ...
    markAsRectifiedButton.addEventListener('click', function () {
        ...
        db.collection("garbage_objects").doc(docId).delete().then(function () {
            // Delete the image from storage
            storage.ref(`/image/garbage_image/${encodeURIComponent(imageName)}`).delete().then(function () {
