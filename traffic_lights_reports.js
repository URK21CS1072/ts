...
db.collection("tl")
    ...
    .then((querySnapshot) => {
        if (!querySnapshot.empty) {
            ...
            console.log("No documents found in 'tl' collection.");
        }
    })
    .catch((error) => {
...
function createTable(data, docId) {
    ...
    const firebaseStorageUrl = `https://firebasestorage.googleapis.com/v0/b/techsavants-bdf20.appspot.com/o/image%2Ftl%2F${encodeURIComponent(imageName)}?alt=media`;
    ...
    markAsRectifiedButton.addEventListener('click', function () {
        ...
        db.collection("tl").doc(docId).delete().then(function () {
            // Delete the image from storage
            storage.ref(`/image/tl/${encodeURIComponent(imageName)}`).delete().then(function () {
